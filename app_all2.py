...
# ==========================================
# NetCDF 時間與空間載入 (相容 days since 解析)
# ==========================================
@st.cache_data
...
# ==========================================
    ...
    if not os.path.exists(nc_path):
        st.error(f"❌ 找不到 NetCDF 檔案：`{nc_path}`\n請檢查 `EXPERIMENT_NC_MAP` 的相對路徑設定！")
        st.stop()

    times, lats, lons = get_nc_time_list(nc_path)
    ...
    colorscale = "Turbid" if var_name == "precipitation" else "Thermal"
    ...
    fig.add_trace(go.Heatmap(z=truth_2d, x=lons[0, :], y=lats[:, 0], colorscale=colorscale, colorbar_x=0.45), row=1, col=1)
    fig.add_trace(go.Heatmap(z=pred_2d, x=lons[0, :], y=lats[:, 0], colorscale=colorscale, colorbar_x=1.0), row=1, col=2)
    fig.add_trace(go.Heatmap(z=input_2d, x=lons[0, :], y=lats[:, 0], colorscale=colorscale, colorbar_x=0.45), row=2, col=1)
    fig.add_trace(go.Heatmap(z=diff_2d, x=lons[0, :], y=lats[:, 0], colorscale="RdBu_r", zmid=0, colorbar_x=1.0), row=2, col=2)

    fig.update_xaxes(title_text="經度", showgrid=False)
    fig.update_yaxes(title_text="緯度", showgrid=False)
