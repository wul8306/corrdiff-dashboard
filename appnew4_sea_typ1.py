...
# 加入這段自訂 CSS 來加寬下拉選單的捲軸
st.markdown(
    """
    <style>
    /* 針對下拉選單清單 (role="listbox") 的捲軸進行加寬 */
    ul[role="listbox"]::-webkit-scrollbar {
        width: 18px !important; /* 捲軸寬度，數字越大越寬 (預設大約只有 6px) */
    }
    
    ul[role="listbox"]::-webkit-scrollbar-track {
        background: #f1f1f1 !important; /* 捲軸軌道的顏色 */
        border-radius: 8px !important;
    }
    
    ul[role="listbox"]::-webkit-scrollbar-thumb {
        background: #888 !important; /* 捲軸本體的顏色 */
        border-radius: 8px !important;
    }
    
    ul[role="listbox"]::-webkit-scrollbar-thumb:hover {
        background: #555 !important; /* 滑鼠移過去時的顏色 */
    }
    </style>
    """,
...
def load_typhoon_dates(file_path="2016_2023_Tydays.txt"):
    ...
    if not os.path.exists(file_path):
        return []
    dates = []
    # 使用 utf-8-sig 可以避免 BOM 字元問題
    with open(file_path, "r", encoding="utf-8-sig") as f:
        for line in f:
            d = line.strip()
            # 若為 20160707 格式則轉換為 2016-07-07
...
tab_selection = st.sidebar.radio(
    ...
    [
        ...
        "🌀 颱風日空間場繪圖 (Typhoon Plot)",  # 新增的 TAB 4
    ],
)
st.sidebar.markdown("---")
...
# ==========================================
# TAB 4: 颱風日空間場繪圖 (Typhoon Plot)
# ==========================================
elif tab_selection == "🌀 颱風日空間場繪圖 (Typhoon Plot)":
    st.title("🌀 颱風日空間場繪圖 (Target / Input / Prediction 對比)")
    ...
    # 3. 載入實驗檔案與變數
    exp_list = list(EXPERIMENT_NC_MAP.keys())
    selected_exp = st.sidebar.selectbox("選擇實驗組別：", options=exp_list, index=0, key="ty_exp")
    nc_path = EXPERIMENT_NC_MAP[selected_exp]
    ...
    all_times = get_nc_time_list(nc_path)
    ...
    # 5. 過濾找出所選日期中「有包含的資料時間點」 (例如 00:00, 06:00 等)
    available_times = [t for t in all_times if t.startswith(selected_ty_date)]

    if not available_times:
        st.warning(f"⚠️ 在 {selected_exp} 實驗中，找不到關於日期 {selected_ty_date} 的資料。")
        st.stop()
    ...
    # 8. 呼叫共用的繪圖函式 (透過 is_typhoon 參數來客製化標題)
    plot_spatial_maps(
        var_name, selected_time_str, selected_exp, 
        lats, lons, land_mask, truth_2d, input_2d, pred_2d, is_typhoon=True
    )
