...
def calculate_rmse_t_season(rmse_t_arr: np.ndarray, times_str: list[str]) -> dict:
    ...
    :param rmse_t_arr: 時間序列 RMSE 陣列
    :param times_str: 對應的時間標籤列表
    :return: 包含五個季節平均 RMSE 的字典
    """
    seasons = ['DJF', 'MA', 'MJ', 'JAS', 'ON']
    
    # 處理沒有時間維度 (STATIC) 的防呆機制
...
def compute_experiment_metrics(nc_path: str, var_name: str = "precipitation",
    ...
    """
    對單一實驗 NetCDF 進行完整指標計算 (包含 Grid-RMSE、RMSE_t、特定季節 RMSE 與多組合 FSS)
    """
...
def init_database(db_path: str):
    ...
    # 新增這四行：先刪除舊的資料表，避免 Schema 不一致的報錯
...
def run_precomputations(exp_nc_map: dict[str, str], db_path: str = "corrdiff_allmetrics_sea.db", var_name: str = "precipitation"):
    """
    執行完整預計算，寫入 SQLite
    """
    ...
    print("\n🎉 所有人預計算指標與散佈圖資料皆已成功更新至 SQLite 資料庫！(包含季節性 RMSE)")
