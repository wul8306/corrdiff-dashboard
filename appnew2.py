...
# ==========================================
# 0. 載入本地中文字型檔 (無需系統安裝權限)
# ==========================================
FONT_PATH = "SourceHanSans-Regular.ttf"  

if os.path.exists(FONT_PATH):
    # 建立 Matplotlib 字型物件
    my_font = fm.FontProperties(fname=FONT_PATH)
    # 將其新增至 Matplotlib 的字型庫中
...
PRECIP_BOUNDS = [0, 1, 2, 6, 10, 15, 20, 30, 40, 50, 70, 90, 110, 130, 150, 200, 300]
PRECIP_COLORS = [
    "#FFFFFF",  # 0~5: 白
    "#A0F0FF",  # 5~10: 淺藍
    "#00A0FF",  # 10~20: 天藍
...
if tab_selection == "📊 全時間平均指標 (RMSE & FSS)":
    ...
    # --- FSS 子選項 2: 固定視窗，畫不同門檻值的 FSS 曲線 ---
    with fss_tab2:
        w_list_tab2 = sorted(df_filtered['fss_window_size'].unique().tolist())
        selected_w_tab2 = st.selectbox("選擇空間視窗大小 (Window Size):", options=w_list_tab2, index=2, key="w_tab2")

        df_fss_line = df_filtered[
            df_filtered['fss_window_size'] == selected_w_tab2
        ...
        fig_fss_line = px.line(
            ...
            color='exp_name',
            markers=True,
            title=f"FSS 隨降雨門檻值變化曲線 (視窗大小: {selected_w_tab2}x{selected_w_tab2})",
            labels={'fss_threshold': '降雨門檻 Threshold (mm/hr)', 'fss_score': '平均 FSS 分數', 'exp_name': '實驗名稱'}
        )
        fig_fss_line.update_layout(yaxis_range=[0, 1.05])
        fig_fss_line.update_xaxes(type='category')  # 讓門檻值以離散類別呈現，便於均勻呈現
...
# ==========================================
    ...
    fig.suptitle(f"{selected_var_label} - {selected_time_str} ({selected_exp})", fontproperties=my_font, fontsize=18, y=0.96)
