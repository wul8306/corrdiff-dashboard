...
def load_grid_scatter_data(
    exp_x, exp_y, var_name="precipitation", db_path=DB_PATH
):
  """從資料庫中直接讀取兩個實驗的格點對應散佈數據與 valid_time 資訊"""
  if not os.path.exists(db_path):
    return None, None, None, None
  conn = sqlite3.connect(db_path)
  cursor = conn.cursor()
  ...
  try:
    cursor.execute(
        "SELECT valid_time, ratio_y_gt_x, x_values, y_values FROM"
        " grid_rmse_scatter WHERE exp_x=? AND exp_y=? AND var_name=?",
...
if tab_selection == "📊 全時間平均指標 (RMSE & FSS)":
  ...
  with fss_tab2:
    ...
    st.plotly_chart(fig_fss_line, use_container_width=True)
...
# ==========================================
  ...
  all_grid_exps = sorted(
      list(set([r[0] for r in combos] + [r[1] for r in combos]))
  )

  col_x, col_y = st.columns(2)
  with col_x:
    exp_x = st.selectbox("選擇 X 軸實驗 (Baseline)", all_grid_exps, index=0)
  with col_y:
    exp_y = st.selectbox(
        "選擇 Y 軸實驗 (Comparison)",
        all_grid_exps,
        index=1 if len(all_grid_exps) > 1 else 0,
    )

  if exp_x == exp_y:
    st.warning("⚠️ 請選擇不同的實驗進行比較。")
  else:
    # 1. 從 DB 直接讀取 valid_time 與 RMSE 散佈陣列
    valid_time, ratio_y_gt_x, x_draw, y_draw = load_grid_scatter_data(
        exp_x, exp_y, var_name="precipitation", db_path=DB_PATH
    )

    if x_draw is not None and len(x_draw) > 0:
      date_info = [valid_time] * len(x_draw)
      ...
      anno_text = (
          f"<b>有效時間: {valid_time}</b><br><b>Y > X 比例:"
          f" {ratio_y_gt_x:.1f}%</b><br>X > Y 比例: {100-ratio_y_gt_x:.1f}%"
      )
